 ...
 * @param asset - The asset as an ArrayBuffer.
 * @return The MIME type of the asset as a string.
 */
export default function parseAssetType(asset: ArrayBuffer) {
    const textDecoder = new TextDecoder();

    // Iterate through the magic numbers to find a match
    for (const magic of MAGIC_NUMBERS) {
        const slice = asset.slice(magic.offset, magic.offset + magic.signature.length);
        const decodedSlice = textDecoder.decode(slice);
        if (decodedSlice === magic.signature)
            return magic.matchType;
    }

    console.warn("Unknown asset strings", textDecoder.decode(asset.slice(0, 20)));
